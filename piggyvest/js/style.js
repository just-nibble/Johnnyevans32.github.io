function calc(){
	...
	if (money === "") {
		...
		if (plan  === 'Piggybank') {
			let interest = 10;
			document.getElementById('result').innerHTML = text + round((amount *  time * interest)/100) + currency + (time * 12) + month;
		}
		if (plan  === 'Safelock') {
			let interest = 15.5;
			document.getElementById('result').innerHTML = text +  round((amount *  time * interest)/100)  + currency + (time * 12) + month;
...
function calc1(){
	...
	if (amount==="" || period===""){
		...
		if (plan  === 'Daily') {
			let time = 30;
			text =  "You will need to save " + round(amount/(time * period)) + " NAIRA " + plan + " for the next " + period + " Months!";
		} 
		if (plan  === 'Weekly') {
			let time = 4.28571429;
			text=  "You will need to save " + round(amount/(time * period))  + " NAIRA " + plan + " for the next " + period + " Months!";
